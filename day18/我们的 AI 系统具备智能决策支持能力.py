...
def save_data():
    with open("data.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "reminders": reminders,
            "review_history": review_history  # 👈 新增
        }, f, ensure_ascii=False, indent=2)


def load_data():
    global next_id, persons, pending_reviews, process_instances, reminders, review_history
    try:
        with open("data.json", "r", encoding="utf-8") as f:
            ...
            review_history = data.get("review_history", [])  # 👈 新增
...
def get_similar_reviews(operation: str, department: str = None, limit: int = 5):
    """查找相似的历史审核记录"""
    similar = []
    ...
    for record in review_history:
        ...
        if department:
            record_dept = record["params"].get("department")
            if record_dept != department:
                continue

        similar.append(record)

    return similar[-limit:] if len(similar) > limit else similar


def calculate_approval_rate(operation: str, department: str = None):
    """计算审批通过率"""
    similar = get_similar_reviews(operation, department, limit=999)
    ...
    total = len(similar)
    approved = len([r for r in similar if r["decision"] == "approved"])
    rejected = len([r for r in similar if r["decision"] == "rejected"])
...
def assess_review_risk(review_id: str) -> dict:
    ...
    # 因素 2：历史通过率
    approval_stats = calculate_approval_rate(operation, department)
    if approval_stats["total"] > 0:
        if approval_stats["approval_rate"] < 50:
            risk_score += 30
            risk_factors.append(f"该操作历史通过率较低（{approval_stats['approval_rate']:.1f}%）")
        elif approval_stats["approval_rate"] > 90:
            risk_score -= 20
            risk_factors.append(f"该操作历史通过率高（{approval_stats['approval_rate']:.1f}%）")

    # 因素 3：部门招聘频率
    if operation == "创建系统账号":
        recent_onboarding = 0
        for proc in process_instances.values():
            if proc["person_data"].get("department") == department:
                proc_age_minutes = calculate_duration_minutes(proc["created_at"])
                if proc_age_minutes < 43200:  # 30天
                    recent_onboarding += 1

        if recent_onboarding > 10:
            risk_score += 20
            risk_factors.append(f"该部门近期入职人数较多（{recent_onboarding}人）")
        elif recent_onboarding < 3:
            risk_score -= 10

    # 因素 4：相似案例
    similar = get_similar_reviews(operation, department, limit=3)
    if similar:
        approved_similar = len([s for s in similar if s["decision"] == "approved"])
        if approved_similar == len(similar):
            risk_score -= 15
            risk_factors.append(f"最近{len(similar)}个相似案例均已批准")
    ...
    if risk_score < 40:
        ...
        recommendation_reason = "风险评估为低风险，符合审批标准"
    elif risk_score < 70:
        recommendation = "review"
        recommendation_text = "⚠️ 建议人工审查"
        recommendation_reason = "存在一定风险因素，需要仔细确认"
    else:
        recommendation = "reject"
        recommendation_text = "❌ 建议拒绝"
        recommendation_reason = "风险评估较高，建议进一步核实"
...
def generate_review_analysis_report(review_id: str) -> str:
    """生成审核分析报告（格式化文本）"""
    review_task = None
    for review in pending_reviews:
        if review["id"] == review_id:
    ...
    person_name = related_process["person_data"]["name"] if related_process else "未知"
    person_age = related_process["person_data"].get("age", "未知")
    department = related_process["person_data"].get("department", "未知")
...
基本信息：
- 审核ID：{review_id}
- 员工：{person_name}，{person_age}岁，{department}
- 审核项：{review_task['operation']}
- 等待时长：{risk_analysis['wait_duration']} 分钟
...
"""
    ...
    stats = risk_analysis['approval_stats']
    if stats['total'] > 0:
        report += f"""
历史数据对比：
- 该操作历史审批：{stats['total']} 次
- 通过率：{stats['approval_rate']:.1f}%
- 通过：{stats['approved']} 次，拒绝：{stats['rejected']} 次
"""

    if risk_analysis['similar_cases'] > 0:
        report += f"- 相似案例：{risk_analysis['similar_cases']} 个\n"

    report += f"""
AI 建议：{risk_analysis['recommendation_text']}
理由：{risk_analysis['recommendation_reason']}
...
def analyze_batch_reviews() -> dict:
    """批量分析所有待审核任务"""
    ...
    for review in pending:
        risk_analysis = assess_review_risk(review["id"])
        ...
        if risk_analysis["risk_level"] == "low":
            low_risk.append(review_info)
        elif risk_analysis["risk_level"] == "medium":
            medium_risk.append(review_info)
        else:
            high_risk.append(review_info)

    batch_suggestion = None
    if len(low_risk) >= 2:
        batch_suggestion = f"建议批量批准 {len(low_risk)} 个低风险审核任务"
    ...
    return {
        ...
        "high_risk": high_risk,
        "batch_approve_suggestion": batch_suggestion
    }
...
def start_process_tool(
    ...
    if process_type == "employee_onboarding":
        ...
        while process["current_step"] < len(process["tasks"]):
            result = execute_current_btask(process)
...
def approve_review_tool(review_id: str) -> str:
    """批准审核任务"""
...
def analyze_review_tool(review_id: str) -> str:
    """分析审核任务的风险并提供决策建议"""
    report = generate_review_analysis_report(review_id)
    return report


def batch_analyze_reviews_tool() -> str:
    """批量分析所有待审核任务"""
    analysis = analyze_batch_reviews()
    ...
    report_lines = [
        ...
        "=" * 50,
        "",
        f"总计：{analysis['total']} 个待审核任务",
        ""
    ]

    if analysis["low_risk"]:
        report_lines.append(f"🟢 低风险任务（{len(analysis['low_risk'])} 个）：")
        for item in analysis["low_risk"]:
            report_lines.append(f"   - {item['review_id']}: {item['operation']}")
        report_lines.append("")

    if analysis["medium_risk"]:
        report_lines.append(f"🟡 中风险任务（{len(analysis['medium_risk'])} 个）：")
        for item in analysis["medium_risk"]:
            report_lines.append(f"   - {item['review_id']}: {item['operation']}")
        report_lines.append("")

    if analysis["high_risk"]:
        report_lines.append(f"🔴 高风险任务（{len(analysis['high_risk'])} 个）：")
        for item in analysis["high_risk"]:
            report_lines.append(f"   - {item['review_id']}: {item['operation']}")
        report_lines.append("")

    if analysis["batch_approve_suggestion"]:
        report_lines.append(f"💡 {analysis['batch_approve_suggestion']}")
...
def smart_approve_tool(review_id: str) -> str:
    ...
    # 执行批准
    result = approve_review_tool(review_id)

    # 如果是高风险，添加警告
    if risk_analysis["risk_level"] == "high":
        return f"⚠️ 警告：该审核为高风险（{risk_analysis['risk_score']}/100）\n{risk_analysis['risk_label']}\n\n{result}"
...
tools = [
    ...
    StructuredTool.from_function(
        ...
        name="monitor_system",
        description="监控系统状态。AI应该在用户打招呼时主动调用"
    ),
    StructuredTool.from_function(
    ...
    StructuredTool.from_function(
        ...
        name="analyze_review",
        description="分析审核任务的风险并提供决策建议。需要提供审核ID"
    ),
    StructuredTool.from_function(
        func=batch_analyze_reviews_tool,
        name="batch_analyze_reviews",
        description="批量分析所有待审核任务，按风险等级分类"
    ),
    StructuredTool.from_function(
        func=smart_approve_tool,
        name="smart_approve",
        description="智能批准审核任务（带风险提示）。优先使用此工具"
    )
]
...
AGENT_SYSTEM_PROMPT = """你是企业业务流程助理，负责管理、监控和智能审核业务流程。
...
审核助理工作流程：
- 用户说"分析审核 review_X" → 调用 analyze_review
- 用户说"有什么需要审批" → 调用 batch_analyze_reviews
- 用户说"批准" → 使用 smart_approve（带风险检查）

主动服务策略：
- 用户打招呼时，先调用 monitor_system
- 发现待审核任务，主动询问是否需要分析
- 批准前，如果用户没要求分析，主动提示风险

示例对话：
用户："有什么需要处理的"
你：[调用 batch_analyze_reviews] "发现 3 个待审核任务..."

用户："分析第一个"
你：[调用 analyze_review] 展示风险分析报告

用户："批准"
你：[调用 smart_approve] 带风险提示的批准

重要规则：
- 永远优先使用 smart_approve
- 分析要客观，基于数据
- 建议要明确
- 用专业但友好的语气"""
...
def root():
    return {
        "message": "DAY 18 AI 审核助理系统",
        "features": ["对话式操作", "主动监控", "智能审核分析", "风险评估"]
    }
...
# 👈 新增：审核分析接口
@app.get("/api/reviews/{review_id}/analyze")
def analyze_review_api(review_id: str):
    """分析单个审核任务"""
    risk_analysis = assess_review_risk(review_id)
    if "error" in risk_analysis:
        return error(risk_analysis["error"])
    return success(risk_analysis)


@app.get("/api/reviews/batch-analyze")
